...
export const PipelineTarefasVisao = ({ funilId, negocios, usuarios, canView, canViewAll, canConclude, onOpenNegocio }: PipelineTarefasVisaoProps) => {
  ...
  return (
    <div>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><h2 className="flex items-center gap-2 text-[15px] font-semibold"><CalendarCheck2 className="h-4 w-4 text-accent" />Agenda comercial</h2><p className="mt-1 text-[10px] text-muted-foreground">Consulte atividades por prazo, responsável, negócio e tipo.</p></div><span className="rounded-full border border-border/25 bg-card/40 px-3 py-1 text-[9px] text-muted-foreground">{tasks.length} tarefa(s)</span></div>
