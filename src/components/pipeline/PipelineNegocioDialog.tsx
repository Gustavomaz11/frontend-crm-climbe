...
import { PipelineReiniciarCadencia } from "./PipelineReiniciarCadencia";
import { PipelineDadosAdicionais } from "./PipelineDadosAdicionais";
import { PipelineConverterVendaDialog } from "./PipelineConverterVendaDialog";
...
type DialogTab = "dados" | "tarefas" | "comentarios" | "historico" | "propostas";

interface PipelineNegocioDialogProps {
  negocio: PipelineNegocio | null;
  preVendas?: boolean;
  canRestartCadence?: boolean;
...
export const PipelineNegocioDialog = ({
  negocio, preVendas = false, canRestartCadence = false,
  initialEtapaId,
  ...
  const closedLead = preVendas && !!negocio && negocio.resultado !== "ABERTO";
  const canSave = !closedLead && !isProcessing && (isCreating || canEdit) && isPipelineNegocioDraftValid(draft);
      ...
      <div className="pointer-events-none fixed inset-0 z-50 flex items-center justify-center p-4">
        <motion.section role="dialog" aria-modal="true" className={`pointer-events-auto flex max-h-[92vh] w-full flex-col overflow-hidden rounded-2xl border border-border/30 bg-card shadow-2xl ${activeTab === "tarefas" ? "max-w-7xl" : "max-w-5xl"}`} initial={{ opacity: 0, scale: 0.97, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.97, y: 10 }}>
          ...
          <div className="overflow-y-auto p-6">
            {negocio && preVendas && canRestartCadence && negocio.resultado === "ABERTO" && ["INTERROMPIDA", "CONCLUIDA", "SEM_RESPOSTA"].includes(negocio.cadenciaStatus || "") && <PipelineReiniciarCadencia id={negocio.id} />}
            {negocio?.preVendaOrigemId && <a className="mb-4 block text-sm text-accent underline" href={`/pipeline-pre-vendas?negocioId=${negocio.preVendaOrigemId}`}>Consultar pré-venda e histórico de origem #{negocio.preVendaOrigemId}</a>}
            {negocio?.negocioGeradoId && <a className="mb-4 block text-sm text-accent underline" href={`/pipeline-vendas?negocioId=${negocio.negocioGeradoId}`}>Abrir negócio gerado #{negocio.negocioGeradoId}</a>}

            {activeTab === "dados" && <><PipelineNegocioFormFields preVendas={preVendas} draft={draft} setDraft={setDraft} empresas={empresas} usuarios={usuarios} etapas={etapas.filter((etapa) => etapa.resultado === "ABERTO" || etapa.id === negocio?.etapaId)} disabled={closedLead || isProcessing || (!isCreating && !canEdit)} /><PipelineDadosAdicionais campanhaFixa={!!negocio?.campanhaOrigemId} draft={draft} setDraft={setDraft} preVendas={preVendas} disabled={closedLead || isProcessing || (!isCreating && !canEdit)} />{negocio?.resultado === "PERDIDO" && negocio.motivoPerdaNome && <div className="mt-5 rounded-xl border border-red-500/20 bg-red-500/5 p-4"><p className="text-[10px] uppercase tracking-wide text-red-500/70">Motivo da perda</p><p className="mt-1 text-xs font-semibold text-red-500">{negocio.motivoPerdaNome}</p>{negocio.observacaoPerda && <p className="mt-2 text-[11px] text-muted-foreground">{negocio.observacaoPerda}</p>}</div>}{showConversion && <div className="mt-5 rounded-xl border border-accent/25 bg-accent/5 p-4"><p className="text-[12px] font-semibold text-foreground">Converter em contrato</p><p className="mt-1 text-[10px] text-muted-foreground">Selecione a empresa cadastrada que será vinculada ao contrato pendente.</p><div className="mt-3 flex gap-2"><select value={conversionEmpresaId} onChange={(event) => setConversionEmpresaId(event.target.value)} className="h-9 flex-1 rounded-lg border border-border/30 bg-background px-3 text-[11px] outline-none"><option value="">Selecione a empresa</option>{empresas.map((empresa) => <option key={empresa.id} value={empresa.id}>{empresa.nome}</option>)}</select><button type="button" disabled={!conversionEmpresaId || isProcessing} onClick={() => onConvert(Number(conversionEmpresaId))} className="rounded-lg bg-accent px-3 text-[11px] font-semibold text-accent-foreground disabled:opacity-50">Gerar contrato</button></div></div>}</>}
            {activeTab === "tarefas" && negocio && <PipelineTarefasPanel negocioId={negocio.id} responsavelId={negocio.responsavelId} initialTaskId={initialTaskId} usuarios={usuarios} canView={canViewTasks} canCreate={canCreateTask} canEdit={canEditTask} canConclude={canConcludeTask} />}
