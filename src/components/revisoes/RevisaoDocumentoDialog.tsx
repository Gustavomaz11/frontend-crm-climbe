...
} from "./reviewAnnotationNavigation";
...
function AuthPage({ revisaoId, versao, pagina, anotacoes, selecionada, onSelecionar }: AuthPageProps) {
  ...
  return (
    <div ref={hostRef} className="space-y-1.5">
      <p className="px-1 text-[10px] text-muted-foreground">Página {pagina}</p>
      <div className="relative overflow-hidden rounded-lg border border-border/25 bg-white shadow-lg">
        {src ? <img src={src} alt={`Página ${pagina}`} className="block w-full" loading="lazy" decoding="async" /> : <div className="flex aspect-[1/1.25] items-center justify-center">{error ? <AlertCircle className="h-5 w-5 text-destructive" /> : <Loader2 className="h-5 w-5 animate-spin text-accent" />}</div>}
        {anotacoes.map((item) => {
          ...
          return (
            <button
              ...
              className={`absolute z-10 cursor-pointer border-2 transition-all duration-200 ${active ? "scale-[1.02] border-accent shadow-[0_0_0_4px_hsl(var(--accent)/.4)]" : "border-foreground/30 hover:border-accent/70"}`}
              style={{ left: `${item.x * 100}%`, top: `${item.y * 100}%`, width: `${item.largura * 100}%`, height: `${item.altura * 100}%`, backgroundColor: `${item.cor}70` }}
            />
          );
        })}
      </div>
...
export function RevisaoDocumentoDialog({ open, tipo, referenciaId, onClose }: Props) {
  ...
  const [anotacaoSelecionada, setAnotacaoSelecionada] = useState<string>();
  ...
  useEffect(() => setAnotacaoSelecionada(undefined), [versaoNumero]);

  function selecionarAnotacao(id: string, source: ReviewAnnotationSource) {
    setAnotacaoSelecionada(id);
  ...
  return <AnimatePresence>{open && <motion.div className="fixed inset-0 z-[70] flex items-center justify-center p-3 sm:p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><motion.div className="absolute inset-0 bg-background/85 backdrop-blur-md" onClick={onClose} /><motion.div className="relative z-10 flex h-[92vh] w-full max-w-7xl flex-col overflow-hidden rounded-2xl border border-border/30 bg-card shadow-2xl" initial={{ scale: .96, y: 15 }} animate={{ scale: 1, y: 0 }} exit={{ scale: .96, y: 15 }}>
    ...
    {loading ? <div className="flex flex-1 items-center justify-center"><Loader2 className="h-7 w-7 animate-spin text-accent" /></div> : !data ? <div className="flex flex-1 items-center justify-center p-6"><div className="max-w-md text-center"><AlertCircle className="mx-auto mb-3 h-8 w-8 text-destructive" /><p className="text-[13px] font-semibold">Revisão indisponível</p><p className="mt-1 text-[11px] text-muted-foreground">{error}</p></div></div> : <div className="grid min-h-0 flex-1 lg:grid-cols-[minmax(0,1fr)_370px]">
      ...
      <aside className="min-h-0 overflow-y-auto border-l border-border/20 p-4 space-y-4">
        ...
        <div className="rounded-xl border border-border/25 p-3"><div className="mb-3 flex items-center justify-between"><div className="flex items-center gap-2"><MessageSquareText className="h-4 w-4 text-accent" /><p className="text-[12px] font-semibold">Comentários</p></div><span className="rounded-full bg-muted/20 px-2 py-0.5 text-[9px]">{versao.anotacoes.length}</span></div>{versao.anotacoes.length ? <div className="space-y-2">{versao.anotacoes.map((item, index) => { const id = reviewAnnotationKey(item.id, item.localId); const active = anotacaoSelecionada === id; return <button key={id} id={reviewAnnotationCommentId(id)} type="button" aria-pressed={active} onClick={() => selecionarAnotacao(id, "comment")} className={`w-full rounded-lg border p-3 text-left transition-all duration-200 ${active ? "scale-[1.01] border-accent bg-accent/10 shadow-[0_0_0_2px_hsl(var(--accent)/.2)]" : "border-border/20 bg-background/40 hover:border-accent/40"}`}><div className="flex items-center gap-2"><span className="h-3 w-3 rounded-sm" style={{ backgroundColor: item.cor }} /><p className="text-[10px] font-semibold">Marcação {index + 1} · página {item.pagina}</p></div><p className="mt-1.5 text-[11px] leading-4 text-muted-foreground">{item.comentario}</p></button>; })}</div> : <p className="text-[11px] text-muted-foreground">Nenhum comentário nesta versão.</p>}</div></>}
