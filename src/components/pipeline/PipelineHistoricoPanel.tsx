...
export const PipelineHistoricoPanel = ({ negocioId, canView }: PipelineHistoricoPanelProps) => {
  ...
  return (
    <div>
      <div><h3 className="text-[13px] font-semibold">Histórico completo</h3><p className="mt-0.5 text-[10px] text-muted-foreground">Linha do tempo auditável de tudo o que aconteceu.</p></div>
      {isLoading ? <p className="py-10 text-center text-[11px] text-muted-foreground">Carregando histórico...</p> : <div className="mt-5 space-y-0">{history.map((event, index) => { const Icon = iconForEvent(event.tipo); return <article key={event.id} className="relative flex gap-3 pb-5"><div className="relative z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-border/30 bg-card text-accent"><Icon className="h-3.5 w-3.5" /></div>{index < history.length - 1 && <div className="absolute bottom-0 left-[15px] top-8 w-px bg-border/25" />}<div className="min-w-0 pt-0.5"><p className="text-[11px] font-medium">{event.descricao}</p><p className="mt-1 text-[9px] text-muted-foreground">{event.usuarioNome} · {formatDateTime(event.criadoEm)}</p></div></article>; })}{history.length === 0 && <p className="py-10 text-center text-[10px] text-muted-foreground">Nenhum evento registrado.</p>}</div>}
    </div>
  );
};
