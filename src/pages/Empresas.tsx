        ...
        <main className={`flex-1 transition-all duration-300 ${sidebarCollapsed ? "ml-[72px]" : "ml-[220px]"}`}>
          <motion.header className="sticky top-0 z-20 h-16 flex items-center justify-between px-6 border-b border-border/20 bg-background/80 backdrop-blur-xl" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
            <div className="flex items-center gap-2 h-9 px-3 rounded-lg border border-border/25 bg-card/30 backdrop-blur-sm text-muted-foreground w-[280px]">
              ...
              <input type="text" placeholder="Buscar empresa..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="flex-1 bg-transparent text-[12px] outline-none placeholder:text-muted-foreground text-foreground" />
            </div>
          ...
          <div className="px-6 pt-6 pb-2 flex items-center justify-between">
            <div>
              ...
              <p className="text-[12px] text-muted-foreground mt-0.5">Gerencie todas as empresas — pendentes e clientes.</p>
            </div>
            <motion.button
          ...
          <div className="px-6 pb-6">
            <motion.div className="rounded-xl border border-border/25 bg-card/40 backdrop-blur-sm overflow-hidden" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }}>
              <div className="divide-y divide-border/10 max-h-[calc(100vh-220px)] overflow-y-auto [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-muted-foreground/20 [&::-webkit-scrollbar-thumb]:rounded-full">
                {isLoading ? (
                  <div className="py-12 text-center text-[12px] text-muted-foreground">Carregando empresas...</div>
                ) : error ? (
                  <div className="py-12 text-center text-[12px] text-destructive">Erro ao carregar empresas</div>
                ) : filtered.length === 0 ? (
                  <div className="py-12 text-center text-[12px] text-muted-foreground">Nenhuma empresa encontrada</div>
                ) : (
                  filtered.map((emp, i) => (
                    <motion.div
                      key={emp.id}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-muted-foreground">{emp.cnpj} · {emp.cidade || "N/A"}</p>
                        </div>
                      </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-muted-foreground/20 [&::-webkit-scrollbar-thumb]:rounded-full">
                <div className="grid grid-cols-2 gap-4">
                  <div className="rounded-xl border border-border/20 bg-background/50 p-5 space-y-3">
                    <p className="text-[10px] text-muted-foreground font-medium tracking-[0.08em] uppercase">Informações</p>
                    <div className="space-y-3">
                      <div><p className="text-[10px] text-muted-foreground">CNPJ</p><p className="text-[13px] text-foreground/80">{selectedEmpresa.cnpj || "N/A"}</p></div>
                      <div><p className="text-[10px] text-muted-foreground">Email</p><p className="text-[13px] text-foreground/80">{selectedEmpresa.email || "N/A"}</p></div>
                      <div><p className="text-[10px] text-muted-foreground">Telefone</p><p className="text-[13px] text-foreground/80">{selectedEmpresa.telefone || "N/A"}</p></div>
                      <div><p className="text-[10px] text-muted-foreground">Cidade</p><p className="text-[13px] text-foreground/80">{selectedEmpresa.cidade || "N/A"}</p></div>
                      <div><p className="text-[10px] text-muted-foreground">Endereço</p><p className="text-[13px] text-foreground/80">{selectedEmpresa.logradouro || "N/A"}{selectedEmpresa.numero ? `, ${selectedEmpresa.numero}` : ""}</p></div>
                      <div><p className="text-[10px] text-muted-foreground">Representante</p><p className="text-[13px] text-foreground/80">{selectedEmpresa.representanteNome || "N/A"}</p></div>
                    </div>
                  </div>
                  <div className="rounded-xl border border-border/20 bg-background/50 p-5 space-y-3">
                    <p className="text-[10px] text-muted-foreground font-medium tracking-[0.08em] uppercase">Ações</p>
                ...
                <div className="rounded-xl border border-border/20 bg-background/50 p-5">
                  <div className="mb-4 flex items-center justify-between gap-3">
                    <div>
                      <p className="text-[10px] font-medium uppercase tracking-[0.08em] text-muted-foreground">Serviços e recebimentos</p>
                      <p className="mt-1 text-[12px] text-muted-foreground">Contratos, parcelas e equipe vinculada por serviço.</p>
                    </div>
                    <div className="text-right">
                      <p className="text-[10px] text-muted-foreground">Próximo recebimento</p>
