...
export const PipelineTarefasVisao = ({ funilId, negocios, usuarios, canView, canConclude, onOpenNegocio }: PipelineTarefasVisaoProps) => {
  ...
  return (
    <div>
      ...
      <div className="mb-4 grid gap-2 rounded-xl border border-border/20 bg-card/25 p-3 sm:grid-cols-3"><label className="flex min-w-0 items-center gap-2"><Filter className="h-3.5 w-3.5 shrink-0 text-muted-foreground/50" /><select value={responsible} onChange={(event) => setResponsible(event.target.value)} className={`${fieldClass} min-w-0 flex-1`}><option value="">Todos os responsáveis</option>{usuarios.map((usuario) => <option key={usuario.id} value={usuario.id}>{usuario.nomeCompleto}</option>)}</select></label><select value={business} onChange={(event) => setBusiness(event.target.value)} className={fieldClass}><option value="">Todos os negócios</option>{negocios.map((negocio) => <option key={negocio.id} value={negocio.id}>{negocio.nomeEmpresa}</option>)}</select><input value={type} onChange={(event) => setType(event.target.value)} className={fieldClass} placeholder="Filtrar por tipo (ex.: Follow-up)" /></div>
